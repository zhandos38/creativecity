                        ...
                        <div class="card-body">
                            <div class="datatable-wrapper table-responsive">
                                <table id="datatable" class="display compact table table-striped table-bordered table-hover">
                                    ...
                                    <tbody>
                                    @forelse($contacts as $v)
                                        <tr>
                                            <td>{{ $v->getType() }}</td>
                                            <td>{{ $v->content_ru }}</td>
                                            <td>{{ $v->created_at }}</td>
                                            <th>
                                                {!! Form::open(['route' => ['admin.contacts.destroy', $v->id], 'method' => 'post']) !!}

                                                    <a class="btn btn-warning mb-1" href="{{ route('admin.contacts.show', $v->id) }}" title="Просмотр">
                                                        <i class="fa fa-eye mr-1"></i>
                                                        Просмотр
                                                    </a>

                                                    <a class="btn btn-info mb-1" href="{{ route('admin.contacts.edit', $v->id) }}" title="Редактировать">
